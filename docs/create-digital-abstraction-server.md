...
![alt text](https://github.com/NI-MeasurementLink-Plug-Ins/adc/blob/main/docs/images/Digital%20Abstraction%20Layer.png)
...
![alt text](images/labviewgrpc.png)
![alt text](https://github.com/NI-MeasurementLink-Plug-Ins/adc/blob/main/docs/images/gRPC%20Server-Client%20Code%20Generator.png)
...
![alt text](https://github.com/NI-MeasurementLink-Plug-Ins/adc/blob/main/docs/images/gRPC%20Template%20Creation%20Utility.png)  
...
![alt text](https://github.com/NI-MeasurementLink-Plug-Ins/adc/blob/main/docs/images/Server%20Library%20creation.png)  
...
9. This dcvmDPI server library is used to communicate with the client APIs (which is part of Measurement Plugins) using gRPC framework. This library holds the responsibility of acquiring the request data from the client and passing the response data to the client whenever client calls happen.
   ...
   ![alt text](https://github.com/NI-MeasurementLink-Plug-Ins/adc/blob/main/docs/images/dcvmDPI_Server%20Library.png)
   
11.	To implement the DUT Interface with the digital instrument, create a separate library which call Server Implementation library. You could name it as dcvmDPI_<DUT_Name>_serverImpl.lvlib.
...
![alt text](https://github.com/NI-MeasurementLink-Plug-Ins/adc/blob/main/docs/images/dcvmDPI_ServerImpl%20Library.png)  
...
1.	Communication from Measurement plugin (gRPC client) to the DUT Interface through the Digital Server (gRPC server) happens as explained below.
    ...
    ![alt text](https://github.com/NI-MeasurementLink-Plug-Ins/adc/blob/main/docs/images/Communication%20from%20Plugin%20to%20DUT%20through%20gRPC%20Digital%20Abstraction%20Layer.png)
3.	The two queues are implemented using a data cluster of ‘Message’ and ‘Message Data’. The ‘Message’ specifies the command or the action to be performed, whereas the ‘Message Data’ specifies the data associated with the command.  
![alt text](https://github.com/NI-MeasurementLink-Plug-Ins/adc/blob/main/docs/images/Message%20Queues%20and%20Queue%20Data%20Format.png)

5.	APIs for creating & destroying message queues, getting message queues in other libraries are implemented in ‘dcvmMessageQueue.lvlib’. This library can be used as such.  
![alt text](https://github.com/NI-MeasurementLink-Plug-Ins/adc/blob/main/docs/images/Message%20Queue%20Library.png)

7.	The ‘Get message queues.vi’ can be used in other VIs to get the message queue references. i.e., Digital engine and gRPC engine.  
![alt text](https://github.com/NI-MeasurementLink-Plug-Ins/adc/blob/main/docs/images/Get%20Message%20Queues.vi.png)
## Step 3: Implement DUT Interface related APIs.
Let’s take a look at the Server Implementation Library in detail in this section.  
    ![alt text](https://github.com/NI-MeasurementLink-Plug-Ins/adc/blob/main/docs/images/Server%20Implementation%20Library.png)  
1. Session.ctl: Update the instrument reference control of the required DUT interface. Bit file reference can be used if a Digital Pattern instrument is used and ignored otherwise.  
    ![alt text](images/Session.ctl.png)
    
2.	Process VI:  In Process.vi, the implementation of interface configurations (using Driver APIs) needs to be done right from initializing the instrument communication to configure the settings to initiate & fetch the measurement to closing the communication. 
   ...
   f.	Acquire: In Acquire.vi, implement the driver APIs needed to acquire data from the DUT. Ensure the acquired data is written to the Data Buffer.vi. Please note that the data buffer expects data in 2D Numeric array (Uint32) format. Ensure to convert the data into the mentioned format in case if it’s in 1D array or any other format.
...
2.	Run Service VI: 
    ...
    ![alt text](https://github.com/NI-MeasurementLink-Plug-Ins/adc/blob/main/docs/images/Run%20service%20vi.jpg)
